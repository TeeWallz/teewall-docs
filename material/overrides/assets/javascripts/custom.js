...
(() => {
  ...
  function getActiveElement() {
    var _a, _b, _c, _d;
    return (_d = (_c = (_b = (_a = document.activeElement) == null ? void 0 : _a.shadowRoot) == null ? void 0 : _b.activeElement) != null ? _c : document.activeElement) != null ? _d : void 0;
  }
      ...
      if (typeof (options == null ? void 0 : options.progress$) !== "undefined") {
        req.addEventListener("progress", (event) => {
          ...
          if (event.lengthComputable) {
            ...
            const length = (_a = req.getResponseHeader("Content-Length")) != null ? _a : 0;
        ...
        return combineLatest([
          ...
          index$.pipe(
            ...
            map(([{ icons, emojis }, mode]) => [
              ...["all", "icons"].includes(mode) ? Object.keys(icons.data) : [],
              ...["all", "emojis"].includes(mode) ? Object.keys(emojis.data) : []
            ])
          )
        ]).pipe(
          map(([{ value }, data]) => (0, import_fuzzaldrin_plus2.filter)(data, value)),
  ...
  function mountIconSearchResult(el, { index$, query$, mode$ }) {
    ...
    return watchIconSearchResult(el, { query$, index$, mode$ }).pipe(
  ...
  function mountIconSearch(el) {
    ...
    const mode$ = new BehaviorSubject("all");
  ...
  function mountSponsorship(el) {
    ...
    sponsorship$.pipe(
      switchMap((sponsorship) => from(total).pipe(
        tap((child) => child.innerText = `$ ${sponsorship.total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} a month`)
      ))
    ).subscribe();
